...
func Run(baseURL, currentVersion, dir, stateDir string) (Result, error) {
	...
	stampPath := filepath.Join(stateDir, "last-update-check.txt")
	...
	manifestBytes, err := fetch(baseURL + "/" + manifestFile)
	if err != nil {
		return Result{}, fmt.Errorf("fetch %s: %w", manifestFile, err)
	}
	// 先验签再解析使用:清单必须能用编译进本二进制的公钥验证通过,否则直接
	// 拒绝——镜像服务器不在信任链里,它只是个文件搬运工。
	sigBytes, err := fetch(baseURL + "/" + manifestFile + ".sig")
	if err != nil {
		return Result{}, fmt.Errorf("fetch %s.sig: %w", manifestFile, err)
	}
